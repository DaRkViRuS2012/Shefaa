...
class DoctorMapViewController: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate {
 ...
 //    var subView: UIView!
    ...
    func prepareView() {
        ...
        self.title = NSLocalizedString("Find a Doctor", comment: "")
    ...
    func loadData(){
        print("load")
        progressHUD.show()
        let url = Globals.mainUrl + "/api/doctors_list_service" + Globals.api.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
        print(url)
    ...
    func setOutLetsMarkers(){
        ...
        for i in 0..<filtered.count {
            ...
            if(splat != 0.0 && splong != 0.0)
            {
                ...
                markersArray[i].title = out.name!
                if((out.doctor_specialization?.count)! > 0){
                    markersArray[i].snippet = out.doctor_specialization?[0]
                }
    ...
    func mapView(_ mapView: GMSMapView, didTapInfoWindowOf marker: GMSMarker) {
        ...
        let doctor = marker.userData as! Doctor
        let vc = DoctorProfileViewController()
        //let vc = DoctorProfileViewController()
        vc.doctor = doctor
        vc.title = doctor.name!
        self.navigationController?.pushViewController(AppMenuController(rootViewController: vc), animated: true)
...
extension DoctorMapViewController: GMSAutocompleteResultsViewControllerDelegate {
